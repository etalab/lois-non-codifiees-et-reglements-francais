...
MODALITÉS DE PRISE EN COMPTE DU SYSTÈME "HÉLIOPAC®"
...
Cette méthode s'applique uniquement aux immeubles collectifs d'habitations, aux bâtiments hôteliers, de restauration, d'hébergement et aux établissements sportifs qui respectent les conditions suivantes :

― des besoins annuels compris entre 500 m³ et 18 000 m³ d'eau chaude sanitaire à 40 °C ;
...
― des capteurs solaires dont la surface est comprise entre 30 m² et 100 m² par pompe à chaleur ;
...
― corriger la consommation d'eau chaude sanitaire en énergie finale ainsi obtenue (<sub>Cecs ef</sub>) par le moteur de calcul par la performance globale annuelle du système (Pgs) pour la part assurée par le système "Héliopac®".
...
3.1. Détermination du cœfficient de performance globale annuelle

du système (Pgs)

Le cœfficient de performance globale annuelle du système est déterminé à partir d'un des tableaux ci-dessous (tableaux 1 à 4) qui se différencient suivant l'usage du bâtiment (immeuble collectif d'habitations ou bâtiment d'hébergement, bâtiment hôtelier, bâtiment de restauration et établissement sportif).

Les données du projet nécessaires à la détermination de ce cœfficient sont :
...
Tableau 1. - Coefficients Pgs et Papp pour les immeubles collectifs d'habitations et les bâtiments d'hébergement

<table>
<tbody>
<tr>
<td rowspan="2" width="46">
<p align="center">Surface de capteur par PAC</p>
</td>
<td rowspan="2" width="53">
<p align="center">Zone climatique</p>
</td>
<td colspan="2" width="95">
<p align="center">Besoin ECS <br/>à 40°C/PAC<br/>500 m3/an</p>
</td>
<td colspan="2" width="97">
<p align="center">Besoin ECS<br/>à 40°C/PAC<br/>1 000 m3/an</p>
</td>
<td colspan="2" width="97">
<p align="center">Besoin ECS<br/>à 40°C/PAC<br/>1 500 m3/an</p>
</td>
<td colspan="2" width="96">
<p align="center">Besoin ECS<br/>à 40°C/PAC<br/>2 000 m3/an</p>
</td>
<td colspan="2" width="97">
<p align="center">Besoin ECS<br/>à 40°C/PAC<br/>2 500 m3/an</p>
</td>
<td colspan="2" width="100">
<p align="center">Besoin ECS<br/>à 40°C/PAC<br/>3 000 m3/an</p>
</td>
</tr>
<tr>
<td width="52">
...
<p align="center">30 m2</p>
</td>
<td width="53">
<p align="center">H1a</p>
</td>
<td width="52">
...
Tableau 2. - Coefficients Pgs et Papp pour les bâtiments hôteliers
...
Tableau 3. - Coefficients Pgs et Papp pour les bâtiments de restauration
...
Tableau 4. - Coefficients Pgs et Papp pour les établissements sportifs
...
Si les valeurs de besoin en eau chaude sanitaire ou celles de la surface effective du projet ne correspondent pas aux valeurs proposées dans les tableaux ci-dessus, on obtient les valeurs de Pgs et de Papp par une interpolation linéaire entre deux lignes et/ou entre deux colonnes.

Lorsque le système "Héliopac®" alimente plusieurs usages différents, le Pgs de chaque usage doit être déterminé sur la base du besoin total en ECS de l'ensemble des usages. Il convient alors d'effectuer une moyenne pondérée des Pgs obtenus au prorata des besoins d'ECS de chaque usage afin de pouvoir calculer la consommation d'ECS en énergie primaire du système "Héliopac® + appoint".

3.2. Saisie du générateur collectif d'eau chaude sanitaire

dans le moteur de calcul Th-C-E
...
Le système utilisé dans le moteur de calcul Th-C-E pour calculer la consommation d'énergie du bâtiment de référence est défini dans le titre II de l'arrêté du 24 mai 2006 et correspond au générateur prévu pour l'appoint.

3.3. Post-traitement pour l'intégration

dans le moteur de calcul Th-C-E

La consommation d'ECS en énergie primaire du système "Héliopac® + appoint" est définie par la formule suivante :

C<sub>ecs</sub> Héliopac = (C<sub>ecs ef</sub>/Pgs) × [2,58 × (1 ― Papp) + C<sub>tep</sub> × Papp]

Où :

C<sub>ecs ef</sub> : consommation d'ECS en énergie finale du projet.

Pgs : performance globale annuelle du système "Héliopac®" ;

Papp : part du besoin produit par l'appoint (en %). Ce coefficient est déterminé à partir de la zone climatique du projet, de la surface de capteurs solaires et du besoin d'eau chaude sanitaire. Les tableaux 1 à 4 donnent les valeurs de ce coefficient.

C<sub>tep</sub> : cœfficient de transformation en énergie primaire de l'énergie d'appoint défini par l'article 35 de l'arrêté du 24 mai 2006 susvisé.

Les corrections à apporter à la consommation d'énergie primaire du projet sont les suivantes :

C<sub>ep</sub> corrigé = C<sub>ep</sub> ― C<sub>ecs</sub> + C<sub>ecs</sub> Héliopac

Où :

C<sub>ep</sub> : consommation en énergie primaire du projet.

C<sub>ecs</sub> Héliopac : consommation d'ECS en énergie primaire du système "Héliopac® + appoint".

C<sub>ecs</sub> : consommation d'ECS en énergie primaire du projet.
